...
public class MainPageTest extends AbstractTestNGSpringContextTests{
    ...
    public void verifyProducts_sortByPrice(){
        //User Sorts the  products with Price (low to high)
        productsContent.sortBy("Price (low to high)");

        //Getting the list of sorted products
        List<Product> productList=productsContent.getProductsList();

        //Creating a list of sorted products price
        List<Float> priceAfter=new ArrayList<>();
        for(int i=0;i < productList.size();i++) {
            priceAfter.add(Float.valueOf(productList.get(i).getPrice().replace("$","")));
        }

        //Converting list to array
        Float[] myArray=new Float[priceAfter.size()];
        priceAfter.toArray(myArray);


        SoftAssert softAssert=new SoftAssert();
        //To check sorting by price the price list of sorted item should be arranged  from low to high
        softAssert.assertTrue(ArrayUtils.isSorted(priceAfter.toArray(myArray)));
